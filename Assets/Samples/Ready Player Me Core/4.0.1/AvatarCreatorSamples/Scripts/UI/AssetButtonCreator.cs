...
namespace ReadyPlayerMe
{
    public class AssetButtonCreator : MonoBehaviour
    {
        ...
        public void ResetUI()
        {
            ...
            // Repeat the same pattern for other dictionaries/lists
    
            // Check if clearButtonByCategory is not null before accessing it
            if (clearButtonByCategory != null)
            {
                foreach (var button in clearButtonByCategory)
                {
                    DestroyImmediate(button.Value.gameObject);
            ...
            // Check if selectedButtonsByCategory is not null before accessing it
            if (selectedButtonsByCategory != null)
            {
                selectedButtonsByCategory.Clear();
            }

            // Check if selectedAssetIdByCategory is not null before accessing it
            if (selectedAssetIdByCategory != null)
            {
                selectedAssetIdByCategory.Clear();
            }
        }
